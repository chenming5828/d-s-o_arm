...
struct gimbal_attitude_t {
    int8_t gimbal_yaw; /*< current gimbal yaw, -45~ 45*/
    int8_t gimbal_roll; /*< current gimbal roll, -128~ 127*/
    int8_t gimbal_pitch; /*< current gimbal pitch, 0 ~ 36*/
...
struct gimbal_control_t {
        ...
        int8_t		procedure_status;//0: unlock;1:detectin;2:tracking
        int8_t		feedback_status;//0:failure;1:success
        uint16_t	width;
        uint16_t	length;
};
...
struct impr_frame_data_ot {
    ...
    struct gimbal_attitude_t gbControl_info;
    int 			 height;
};
...
typedef enum {
    IMPR_DT_EVENT_IMU = 1,
} impr_data_type_event_t;


struct impr_event_data {
    impr_data_type_event_t type;
    union {
        struct impr_event_data_imu imu;
    } u;
};
...
int impr_process_event(void *phandle, struct impr_event_data *event_data);
...
typedef int (*impr_process_event_fn)(void *phandle, struct impr_event_data *event_data);
...
struct impr_intf {
    ...
    impr_process_event_fn         fn_process_event;
};
